...
/**
 * A fast, unsynchronized, chunk-based long buffer for storing LCs and VTD. Creation date: (7/17/03
 * 6:07:46 PM)
 */
public class FastLongBuffer implements ILongBuffer {
  ...
  public void append(long[] long_array) {
    ...
    if (size + long_array.length < capacity) {
      ...
      if (size + long_array.length < lastBufferIndex + 1 << exp) {
        ...
        int z;
        for (z = 1; z <= k; z++) {
          System.arraycopy(long_array, offset, bufferArrayList.oa[lastBufferIndex + z], 0,
              pageSize);
          offset += pageSize;
        }
        // copy the last part
    ...
    int n = (long_array.length + size >> exp) + ((long_array.length + size & r) > 0 ? 1 : 0)
        - (capacity >> exp);
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i < n - 1) {
        ...
        // last page
        System.arraycopy(long_array, pageSize * i + capacity - size, newBuffer, 0,
  ...
  public void append(long i) {
    ...
    // int lastBufferIndex;
    /*
     * if (bufferArrayList.size == 0) { lastBuffer = new long[pageSize];
     * bufferArrayList.add(lastBuffer); capacity = pageSize; } else { lastBufferIndex =
     * Math.min((size>>exp),//+(((size&r)==0)? 0:1), bufferArrayList.size - 1); lastBuffer =
     * (long[]) bufferArrayList.oa[lastBufferIndex]; //lastBuffer = (long[])
  ...
  public long[] toLongArray() {
    if (size > 0) {
      ...
      int array_offset = 0;
      for (int i = 0; s > 0; i++) {
        System.arraycopy(bufferArrayList.oa[i], 0, resultArray, array_offset,
  ...
  /**
   * set teh size of long buffer to zero, capacity untouched so long buffer can be reused without
  ...
  /**
   * Set the size of FastLongBuffer to newSz if newSz is less than the capacity, otherwise return
   * false
   *
   * @param newSz
