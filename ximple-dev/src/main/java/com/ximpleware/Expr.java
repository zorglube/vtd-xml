...
abstract public class Expr {
  ...
  /*
   * final protected int getStringIndex(VTDNav vn){ int a = -1; vn.push2(); int size =
   * vn.contextStack2.size; try { a = evalNodeSet(vn); if (a != -1) { int t = vn.getTokenType(a); if
   * (t == VTDNav.TOKEN_ATTR_NAME) { a++; } else if (vn.getTokenType(a) ==
   * VTDNav.TOKEN_STARTING_TAG) { a = -2; }else if (t == VTDNav.TOKEN_PI_NAME) { a++; } } } catch
   * (Exception e) {
   * 
   * } vn.contextStack2.size = size; reset(vn); vn.pop2(); return a; }
   */
  ...
  protected int computeDataSize(VTDNav vn) {
    ...
    if (vn.shallowDepth) {
      return switch (i) {
        case -1, 0 -> vn.vtdSize;
        case 1 -> vn.vtdSize / vn.l1Buffer.size;
        case 2 -> vn.vtdSize / vn.l2Buffer.size;
        default -> vn.vtdSize / vn.l3Buffer.size;
      };
    } else {
      ...
      return switch (i) {
        ...
        case 1 -> vn.vtdSize / vn.l1Buffer.size;
        case 2 -> vn.vtdSize / vn.l2Buffer.size;
        case 3 -> vn.vtdSize / vnl.l3Buffer.size;
        case 4 -> vnl.vtdSize / vnl.l4Buffer.size;
        default -> vnl.vtdSize / vnl.l5Buffer.size;
      };
    }
  }
}
