...
/**
 * XimpleWare's AutoPilotHuge implementation encapsulating node iterator and XPath. AutoPilotHuge is
 * an extended VTD edition of AutoPilot supporting 256 GigByte XML file
 *
 */
public class AutoPilotHuge {
  ...
  static private Hashtable<String, String> nsHash;
  // private parser p;
  ...
  /**
   * Use this constructor for delayed binding to VTDNavHuge which allows the reuse of XPath
   * expression
   *
   */
  ...
  /**
   * Bind resets the internal state of AutoPilotHuge so one can attach a VTDNavHuge object to the
   * autopilot
   *
   * @param vnv
   *
   ...
   *
   * @return boolean
   * @exception com.ximpleware.extended.NavExceptionHuge See description in method toElement() in
   *            VTDNavHuge class.
   */
  public boolean iterate() throws PilotExceptionHuge, NavExceptionHuge {
    switch (iter_type) {
      ...
      case FOLLOWING:
        ...
        while (true) {
          if (vn.toElement(VTDNavHuge.NS)) {
            if (special || vn.matchElement(name)) {
              return true;
            }
            return vn.iterate_following(name, special);
          }
          if (!vn.toElement(VTDNavHuge.P)) {
            // return vn.iterate_following(name, special);
            return false;
          }
        }
