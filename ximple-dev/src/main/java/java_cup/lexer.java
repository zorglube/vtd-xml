...
/**
 * This class implements a small scanner (aka lexical analyzer or lexer) for the JavaCup
 * specification. This scanner reads characters from standard input (System.in) and returns integers
 * corresponding to the terminal number of the next Symbol. Once end of input is reached the EOF
 * Symbol is returned on every subsequent call.
 * <p>
 ...
 * <p>
 *
 * In addition to the scanner proper (called first via init() then with next_token() to get each
 * Symbol) this class provides simple error and warning routines and keeps a count of errors and
 * warnings that is publicly accessible.
 * <p>
 *
...
public class lexer {
  ...
  /**
   * Table of keywords. Keywords are initially treated as identifiers. Just before they are returned
   * we look them up in this table to see if they match one of the keywords. The string of the name
   * is the key here, which indexes Integer objects holding the symbol number.
   */
  protected static Hashtable<String, Integer> keywords = new Hashtable<>(23);

  /* . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . */

  /**
   * Table of single character symbols. For ease of implementation, we store all unambiguous single
   * character Symbols in this table of Integer objects keyed by Integer objects with the numerical
   * value of the appropriate char (currently Character objects have a bug which precludes their use
   * in tables).
   */
  protected static Hashtable<Integer, Integer> char_symbols = new Hashtable<>(11);
  ...
  /**
   * Initialize the scanner. This sets up the keywords and char_symbols tables and reads the first
  ...
  /**
   * Advance the scanner one character in the input stream. This moves next_char2 to next_char and
   * then reads a new next_char2.
   */
  ...
  /**
   * Emit an error message. The message will be marked with both the current line number and the
   * position in the line. Error messages are printed on standard error (System.err).
   *
  ...
  /**
   * Emit a warning message. The message will be marked with both the current line number and the
   * position in the line. Messages are printed on standard error (System.err).
   *
   * @param message the message to print.
   */
  ...
  protected static boolean id_start_char(int ch) {
    /*
     * allow for % in identifiers. a hack to allow my %prec in. Should eventually make lex spec for
     * this frankf
     */
  ...
  protected static int find_single_char(int ch) {
    Integer result = char_symbols.get(Integer.valueOf((char) ch));
    if (result == null) {
      return -1;
    }
    return result;
  ...
  /**
   * Swallow up a code string. Code strings begin with "{:" and include all characters up to the
   * first occurrence of ":}" (there is no way to include ":}" inside a code string). The routine
   * returns a String object suitable for return by the scanner.
   */
   ...
   * followed by zero or more letters, numbers, underscores or dollar signs. This routine returns a
   * String suitable for return by the scanner.
   */
  ...
  /**
   * Return one Symbol. This is the main external interface to the scanner. It consumes sufficient
   * characters to determine the next input Symbol and returns it. To help with debugging, this
   * routine actually calls real_next_token() which does the work. If you need to debug the parser,
   * this can be changed to call debug_next_token() which prints a debugging message before
  ...
  /**
   * Debugging version of next_token(). This routine calls the real scanning routine, prints a
   * message on System.out indicating what the Symbol is, then returns it.
   */
  ...
  /**
   * The actual routine to return one Symbol. This is normally called from next_token(), but for
   * debugging purposes can be called indirectly from debug_next_token().
   */
  protected static Symbol real_next_token() throws java.io.IOException {
    ...
    for (;;) {
      ...
      /*
       * find a "%prec" string and return it. otherwise, a '%' was found, which has no right being
       * in the specification otherwise
       */
      if (next_char == '%') {
        advance();
      ...
      emit_warn("Unrecognized character '" + Character.valueOf((char) next_char) + "'(" + next_char
          + ") -- ignored");
