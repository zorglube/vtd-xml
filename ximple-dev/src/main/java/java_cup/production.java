...
/**
 * This class represents a production in the grammar. It contains a LHS non terminal, and an array
 * of RHS symbols. As various transformations are done on the RHS of the production, it may shrink.
 * As a result a separate length is always maintained to indicate how much of the RHS array is still
 * valid.
 * <p>
 *
 * I addition to construction and manipulation operations, productions provide methods for factoring
 * out actions (see remove_embedded_actions()), for computing the nullability of the production
 * (i.e., can it derive the empty string, see check_nullable()), and operations for computing its
 * first set (i.e., the set of terminals that could appear at the beginning of some string derived
 * from the production, see check_first_set()).
 *
...
public class production {
  ...
  /**
   * Full constructor. This constructor accepts a LHS non terminal, an array of RHS parts (including
   * terminals, non terminals, and actions), and a string for a final reduce action. It does several
   * manipulations in the process of creating a production object. After some validity checking it
   * translates labels that appear in actions into code for accessing objects on the runtime parse
   * stack. It them merges adjacent actions if they appear and moves any trailing action into the
   * final reduce actions string. Next it removes any embedded actions by factoring them out with
   * new action productions. Finally it assigns a unique index to the production.
   * <p>
   *
   * Factoring out of actions is accomplished by creating new "hidden" non terminals. For example if
   ...
   * </pre>
   *
   * (where X is a unique new non terminal). This has the effect of placing all actions at the end
   * where they can be handled as part of a reduce by the parser.
   */
  public production(non_terminal lhs_sym, production_part rhs_parts[], int rhs_l, String action_str)
    ...
    /*
     * I'm not translating labels anymore, I'm adding code to declare labels as valid variables.
     * This way, the users code string is untouched 6/96 frankf
     */

    /*
     * check if the last part of the right hand side is an action. If it is, it won't be on the
     * stack, so we don't want to count it in the rightlen. Then when we search down the stack for a
    ...
    /*
     * Why does this run through the right hand side happen over and over? here a quick combination
     * of two prior runs plus one I wanted of my own frankf 6/25/96
     */
    ...
    /*
     * now action string is really declaration string, so put it in front! 6/14/96 frankf
     */
    if (action_str == null) {
      action_str = "";
    }
  ...
  /*
   * Constructor with precedence and associativity of production contextually define
   */
  public production(non_terminal lhs_sym, production_part rhs_parts[], int rhs_l, String action_str,
   ...
   */
  public production(non_terminal lhs_sym, production_part rhs_parts[], int rhs_l, int prec_num,
  ...
  /**
   * An action_part containing code for the action to be performed when we reduce with this
   * production.
   */
  ...
  /**
   * First set of the production. This is the set of terminals that could appear at the front of
   * some string derived from this production.
   */
  ...
  protected String make_declaration(String labelname, String stack_type, int offset) {
    ...
    if (emit.lr_values()) {
      ret = "\t\tint " + labelname + "left = ((java_cup.runtime.Symbol)" + emit.pre("stack")
          + ".elementAt(" + emit.pre("top") + "-" + offset + ")).left;\n" + "\t\tint " + labelname
          + "right = ((java_cup.runtime.Symbol)" + emit.pre("stack") + ".elementAt("
  ...
  protected String declare_labels(production_part rhs[], int rhs_len, String final_action) {
    ...
    for (pos = 0; pos < rhs_len; pos++) {
      if (!rhs[pos].is_action()) {
        ...
        if (part.label() != null) {
          declaration.append(
              make_declaration(part.label(), part.the_symbol().stack_type(), rhs_len - pos - 1));
        }
      }
    }
  ...
  /**
   * Remove all embedded actions from a production by factoring them out into individual action
   * production using new non terminals. if the original production was:
   *
   * <pre>
  ...
  /*
   * the declarations added to the parent production are also passed along, as they should be
   * perfectly valid in this code string, since it was originally a code string in the parent, not
   * on its own. frank 6/20/96
   */
  ...
  /**
   * Check to see if the production (now) appears to be nullable. A production is nullable if its
   * RHS could derive the empty string. This results when the RHS is empty or contains only non
   * terminals which themselves are nullable.
   */
  ...
  /**
   * Update (and return) the first set based on current NT firsts. This assumes that nullability has
   * already been computed for all non terminals and productions.
   */
  ...
  public String toString() {
    ...
    try {
      ...
      /*
       * crash on internal error since we can't throw it from here (because superclass does not
       * throw anything.
       */
      e.crash();
      result = null;
  ...
  public String to_simple_string() throws internal_error {
    StringBuilder result =
        new StringBuilder().append(lhs() != null ? lhs().the_symbol().name() : "NULL_LHS");
