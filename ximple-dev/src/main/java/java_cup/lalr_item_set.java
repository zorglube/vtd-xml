...
/**
 * This class represents a set of LALR items. For purposes of building these sets, items are
 * considered unique only if they have unique cores (i.e., ignoring differences in their lookahead
 * sets).
 * <p>
 *
 * This class provides fairly conventional set oriented operations (union, sub/super-set tests,
 * etc.), as well as an LALR "closure" operation (see compute_closure()).
 *
...
public class lalr_item_set {
  ...
  /**
   * Add a singleton item, merging lookahead sets if the item is already part of the set. returns
   * the element of the set that was added or merged into.
   *
  ...
  /**
   * Compute the closure of the set using the LALR closure rules. Basically for every item of the
   * form:
   * 
   * <pre>
   *    [L ::= a *N alpha, l]
   * </pre>
   *
   * (where N is a a non terminal and alpha is a string of symbols) make sure there are also items
   * of the form:
   * 
   * <pre>
   ...
   * </pre>
   *
   * corresponding to each production of N. Items with identical cores but differing lookahead sets
   * are merged by creating a new item with the same core and the union of the lookahead sets (the
   * LA in LALR stands for "lookahead merged" and this is where the merger is). This routine assumes
   * that nullability and first sets have been computed for all productions before it is called.
   */
