...
/**
 * This class serves as the main driver for the JavaCup system. It accepts user options and
 * coordinates overall control flow. The main flow of control includes the following activities:
 * <ul>
 ...
 * <dt>-noscanner
 * <dd>don't refer to java_cup.runtime.Scanner in the parser (for compatibility with old runtimes)
 * <dt>-version
...
public class Main {
  ...
  /**
   * User option -- should generator suppress references to java_cup.runtime.Scanner for
  ...
  public static void main(String argv[])
    ...
    /*
     * If there were errors during the run, exit with non-zero status (makefile-friendliness). --CSA
     */
    if (lexer.error_count != 0) {
      System.exit(100);
    }
  ...
  protected static void open_files() {
    File fil;
    String out_name = emit.parser_class_name + ".java";
    ...
    try {
      parser_class_file =
          new PrintWriter(new BufferedOutputStream(new FileOutputStream(fil), 4096));
    } catch (Exception e) {
  ...
  protected static void parse_grammar_spec() throws java.lang.Exception {
    ...
    try {
      ...
      /*
       * something threw an exception. catch it and emit a message so we have a line number to work
       * with, then re-throw it
       */
  ...
  protected static void check_unused() {
    ...
    for (Enumeration<?> t = terminal.all(); t.hasMoreElements();) {
      ...
      /* or error */
      if ((term == terminal.EOF) || (term == terminal.error)) {
        continue;
      }
      ...
      if (term.use_count() == 0) {
        ...
        if (!emit.nowarn) {
          System.err
              .println("Warning: Terminal \"" + term.name() + "\" was declared but never used");
          lexer.warning_count++;
        }
      }
    }
    ...
    for (Enumeration<?> n = non_terminal.all(); n.hasMoreElements();) {
      ...
      if (nt.use_count() == 0) {
        ...
        if (!emit.nowarn) {
          System.err
              .println("Warning: Non terminal \"" + nt.name() + "\" was declared but never used");
          lexer.warning_count++;
        }
      }
    }
  ...
  protected static void emit_parser() throws internal_error {
    ...
    emit.parser(parser_class_file, action_table, reduce_table, start_state.index(),
        emit.start_production, opt_compact_red, suppress_scanner);
  }
  ...
  /**
   * Emit a long summary message to standard error (System.err) which summarizes what was found in
   * the specification, how many states were produced, how many conflicts were found, etc. A
   * detailed timing summary is also produced if it was requested by the user.
   *
  ...
  protected static void emit_summary(boolean output_produced) {
    ...
    System.err
        .print(non_terminal.number() + " non-terminal" + plural(non_terminal.number()) + ", and ");
    System.err
        .println(production.number() + " production" + plural(production.number()) + " declared, ");
    System.err.println("  producing " + lalr_state.number() + " unique parse states.");

    /* unused symbols */
    System.err.println("  " + emit.unused_term + " terminal" + plural(emit.unused_term)
        + " declared but not used.");
    System.err.println("  " + emit.unused_non_term + " non-terminal" + plural(emit.unused_term)
        + " declared but not used.");

    /* productions that didn't reduce */
    System.err.println(
        "  " + emit.not_reduced + " production" + plural(emit.not_reduced) + " never reduced.");

    /* conflicts */
    System.err.println("  " + emit.num_conflicts + " conflict" + plural(emit.num_conflicts)
        + " detected" + " (" + expect_conflicts + " expected).");
  ...
  protected static void show_times() {
    ...
    if (nullability_end != 0 && check_end != 0) {
      System.err
          .println("        Nullability  " + timestr(nullability_end - check_end, total_time));
    }
    if (first_end != 0 && nullability_end != 0) {
      System.err
          .println("        First sets   " + timestr(first_end - nullability_end, total_time));
    }
    ...
    if (reduce_check_end != 0 && table_end != 0) {
      System.err
          .println("        Checking     " + timestr(reduce_check_end - table_end, total_time));
    }
  ...
  /**
   * Helper routine to format a decimal based display of seconds and percentage of total time given
   * counts of milliseconds. Note: this is broken for use with some instances of negative time
   * (since we don't use any negative time here, we let if be for now).
   *
