...
public class FastLongBuffer implements ILongBuffer {
   ...
   *
   */
  private ArrayList<long[]> bufferArrayList;
  ...
  public void append(long[] long_array) {
    ...
    if (size + long_array.length < capacity) {
      ...
      if (size + long_array.length < lastBufferIndex + 1 << exp) {
        ...
        int z;
        for (z = 1; z <= k; z++) {
          System.arraycopy(long_array, offset, bufferArrayList.get(lastBufferIndex + z), 0,
              pageSize);
          offset += pageSize;
        }
        // copy the last part
        System.arraycopy(long_array, offset, bufferArrayList.get(lastBufferIndex + z), 0, l & r);
      }
  ...
  public long[] getLongArray(int startingOffset, int len) {
    ...
    if (first_index == last_index) {
      ...
      for (int i = first_index; i <= last_index; i++) {
        long[] currentChunk = bufferArrayList.get(i);
  ...
  public long longAt(int index) {
    ...
    // int offset = index % r;
    int offset = index & r;
    return bufferArrayList.get(pageNum)[offset];
  }
  ...
  public int upper32At(int index) {
    ...
    int pageNum = index >> exp;
    int offset = index & r;
    return (int) ((bufferArrayList.get(pageNum)[offset] & 0xffffffffL << 32) >> 32);
