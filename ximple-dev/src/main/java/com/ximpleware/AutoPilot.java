...
/*
 * All licenses to any parties in litigation with XimpleWare have been expressly terminated. No new
 * license, and no renewal of any revoked license, is granted to those parties as a result of
 * re-downloading software from this or any other website
 */
...
public class AutoPilot {
  ...
  static protected Hashtable<String, String> nsHash;
  ...
  static private Hashtable<String, Expr> symbolHash;
  // static int count=0;
  ...
  /**
   * Bind is to replace rebind() and setVTDNav() It resets the internal state of AutoPilot so one
   * can attach a VTDNav object to the autopilot
   *
   * @param vnv
   *
   */
  ...
  public void declareVariableExpr(String varName, String varExpr) throws XPathParseException {
    try {
      ...
      System.out.println(
          "Syntax error after or around the end of  ==>" + varExpr.substring(0, e.getOffset()));
      throw e;
    } catch (Exception e) {
  ...
  /**
   * Iterate over all the selected element nodes in document order. Null element name allowed,
   * corresponding to node() in xpath Creation date: (12/4/03 5:25:42 PM)
   *
   * @return boolean
  ...
  public boolean iterate() throws PilotException, NavException {
    ...
    switch (iter_type) {
      case SIMPLE:
        ...
        /*
         * if (elementName == null) throw new PilotException( " Element name not set ");
         */
        if (vn.atTerminal) {
          return false;
        }
    ...
    while (index < size) {
      ...
      if (type == VTDNav.TOKEN_ATTR_NAME || type == VTDNav.TOKEN_ATTR_NS) {
        ...
        if (type == VTDNav.TOKEN_ATTR_NS
            && (name.equals("*") || vn.matchRawTokenString(index, name2))
            && checkNsUniqueness(index)) {
          vn.LN = index;
          vn.atTerminal = true;
          return index;
        }
  ...
  /**
   * This method is meant to be called after calling selectAttr() or selectAttrNs(), it will return
   * the vtd index attribute name or -1 if there is none left
   *
  ...
  /**
   * Select the element name before iterating. "*" matches every element Creation date: (12/4/03
   * 5:51:31 PM)
   *
  ...
  /**
   * Select the element name (name space version) before iterating. URL, if set to *, matches every
   * namespace URL, if set to null, indicates the namespace is undefined. localname, if set to *,
   * matches any localname Creation date: (12/4/03 6:05:19 PM)
   *
   * @param ns_URL String
   * @param ln String
   */
  ...
  /**
   * Select all elements along the following axis, without ns, null selects every elements and
   * documents
   *
   * @param en
   */
  ...
  /**
   * Select all elements along the preceding axis as defined in XPath This is the namespace aware
   * version
   *
   * @param ns_URL
   * @param ln
  ...
  public void selectXPath(String s) throws XPathParseException {
    try {
      ...
      System.out
          .println("Syntax error after or around the end of ==>" + s.substring(0, e.getOffset()));
      throw e;
    } catch (Exception e) {
      throw new XPathParseException("Error occurred");
    }
  ...
  /**
   * This method returns the next node in the nodeset it returns -1 if there is no more node Afer
   * finishing evaluating, don't forget to <em> reset the xpath </em>
   *
  ...
  /**
   * Setspecial is used by XPath evaluator to distinguish between node() and * node() corresponding
   * to b= true;
   *
   * @param b
   */
  ...
  /**
   * set state to false to disable caching, which by default is enabled This method will turn on the
   * internal caching feature during xpath evaluation, which is to store the evaluation results of
   * absolute expressions when they resides in a predicate examples: //a[//bb=//cc] //bb and //cc
   * are cached internally to avoid repeitive computation..
   *
   * @param state
   */
