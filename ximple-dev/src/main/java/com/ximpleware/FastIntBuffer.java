...
public class FastIntBuffer implements IIntBuffer {
  ...
  /**
   * Constructor with adjustable buffer page size of the value bfz The actually page size is
   * 1&lt;&lt;e
   *
  ...
  public void append(int[] int_array) {
    /*
     * if (int_array == null) { throw new NullPointerException(); }
     */
    ...
    if (size + int_array.length < capacity) {
      ...
      if (size + int_array.length < lastBufferIndex + 1 << exp) {
        ...
        int z;
        for (z = 1; z <= k; z++) {
          System.arraycopy(int_array, offset, bufferArrayList.get(lastBufferIndex + z), 0,
              pageSize);
          offset += pageSize;
        }
        // copy the last part
    ...
    int n = (int_array.length + size >> exp) + ((int_array.length + size & r) > 0 ? 1 : 0)
        - (capacity >> exp);
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i < n - 1) {
        ...
        // last page
        System.arraycopy(int_array, pageSize * i + capacity - size, newBuffer, 0,
            int_array.length + size - capacity - pageSize * i);
      }
  ...
  public void append(int i) {
    ...
    // int lastBufferIndex;
    /*
     * if (bufferArrayList.size == 0) { lastBuffer = new int[pageSize];
     * bufferArrayList.add(lastBuffer); capacity = pageSize; } else { lastBufferIndex =
     * Math.min((size>>exp),//+(((size&r)==0)? 0:1), bufferArrayList.size - 1); lastBuffer = (int[])
     * bufferArrayList.oa[lastBufferIndex]; //lastBuffer = (int[])
     * bufferArrayList.get(bufferArrayList.size() - 1); }
     */
    if (size < capacity) {
      // get the last buffer from the bufferListArray
      // obtain the starting offset in that buffer to which the data is to be copied
  ...
  public void modifyEntry(int index, int newValue) {
    ...
    /*
     * if (index > size - 1) { throw new IndexOutOfBoundsException(" index out of range"); }
     */
  ...
  public int[] toIntArray() {
    if (size > 0) {
      ...
      int array_offset = 0;
      for (int i = 0; s > 0; i++) {
        System.arraycopy(bufferArrayList.get(i), 0, resultArray, array_offset,
  ...
  /**
   * set the size of int buffer to zero, capacity untouched so int buffer can be reused without any
   * unnecessary and additional allocation
   *
   */
  ...
  /**
   * Set the size of FastIntBuffer to newSz if newSz is less than the capacity, otherwise return
   * false
   *
   * @param newSz
   ...
   *
   * @param order (as of version 2.9) it can be either ASCENDING or DESCENDING
   */
