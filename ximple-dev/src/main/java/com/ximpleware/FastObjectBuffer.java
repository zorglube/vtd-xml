...
public class FastObjectBuffer implements IObjectBuffer {
  ...
  public void append(Object[] obj_array) {
    ...
    if (size + obj_array.length < capacity) {
      ...
      if (size + obj_array.length < lastBufferIndex + 1 << exp) {
        ...
        int z;
        for (z = 1; z <= k; z++) {
          System.arraycopy(obj_array, offset, bufferArrayList.get(lastBufferIndex + z), 0,
              pageSize);
          offset += pageSize;
        }
        // copy the last part
    ...
    int n = (obj_array.length + size >> exp) + ((obj_array.length + size & r) > 0 ? 1 : 0)
        - (capacity >> exp);
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i < n - 1) {
        ...
        // last page
        System.arraycopy(obj_array, pageSize * i + capacity - size, newBuffer, 0,
            obj_array.length + size - capacity - pageSize * i);
      }
  ...
  final public void append(Object obj) {
    ...
    // int lastBufferIndex;
    /*
     * if (bufferArrayList.size == 0) { lastBuffer = new Object[pageSize];
     ...
     * (Object[]) bufferArrayList.oa[lastBufferIndex]; //lastBuffer = (int[])
  ...
  /**
   * set the size of object buffer to zero, capacity untouched so object buffer can be reused
   * without any unnecessary and additional allocation
   *
   */
