...
/**
 * This class handles emitting generated code for the resulting parser. The various parse tables
 * must be constructed, etc. before calling any routines in this class.
 * <p>
 *
 ...
 * <dt>action class
 * <dd>this non-public class contains code to invoke all the user actions that were embedded in the
 * parser specification.
 * <dt>parser class
 * <dd>the specialized parser class consisting primarily of some user supplied general and
 * initialization code, and the parse tables.
 * </dl>
 * <p>
 *
 ...
 * <dt>action table
 * <dd>for each state of the parse machine, gives the action to be taken (shift, reduce, or error)
 * under each lookahead symbol.<br>
 * <dt>reduce-goto table
 * <dd>when a reduce on a given production is taken, the parse stack is popped back a number of
 * elements corresponding to the RHS of the production. This reveals a prior state, which we
 * transition out of under the LHS non terminal symbol for the production (as if we had seen the LHS
 * symbol rather than all the symbols matching the RHS). This table is indexed by non terminal
 * numbers and indicates how to make these transitions.
 * </dl>
 * <p>
 *
 * In addition to the method interface, this class maintains a series of public global variables and
 * flags indicating how misc. parts of the code and other output is to be produced, and counting
 * things such as number of conflicts detected (see the source code and public variables below for
 * more details).
 * <p>
 *
...
/*
 * Major externally callable routines here include: symbols - emit the symbol constant class parser
 * - emit the parser class
 *
 * In addition the following major internal routines are provided: emit_package - emit a package
 * declaration emit_action_code - emit the class containing the user's actions emit_production_table
 * - emit declaration and init for the production table do_action_table - emit declaration and init
 * for the action table do_reduce_table - emit declaration and init for the reduce-goto table
 *
 * Finally, this class uses a number of public instance variables to communicate optional parameters
 * and flags used to control how code is generated, as well as to report counts of various things
 * (such as number of conflicts detected). These include:
 *
 * prefix - a prefix string used to prefix names that would otherwise "pollute" someone else's name
 * space. package_name - name of the package emitted code is placed in (or null for an unnamed
 * package. symbol_const_class_name - name of the class containing symbol constants.
 * parser_class_name - name of the class for the resulting parser. action_code - user supplied
 * declarations and other code to be placed in action class. parser_code - user supplied
 * declarations and other code to be placed in parser class. init_code - user supplied code to be
 * executed as the parser is being initialized. scan_code - user supplied code to get the next
 * Symbol. start_production - the start production for the grammar. import_list - list of imports
 * for use with action class. num_conflicts - number of conflicts detected. nowarn - true if we are
 * not to issue warning messages. not_reduced - count of number of productions that never reduce.
 * unused_term - count of unused terminal symbols. unused_non_term - count of unused non terminal
 * symbols. _time - a series of symbols indicating how long various sub-parts of code generation
 * took (used to produce optional time reports in main).
 */
...
public class emit {
  ...
  /**
   * Emit code for the symbol constant class, optionally including non terms, if they have been
   * requested.
   *
   * @param out stream to produce output on.
  ...
  protected static void emit_action_code(PrintWriter out, production start_prod)
    ...
    for (Enumeration p = production.all(); p.hasMoreElements();) {
      ...
      /*
       * make the variable RESULT which will point to the new Symbol (see below) and be changed by
       * action code 6/13/96 frankf
       */
      out.println("              " + prod.lhs().the_symbol().stack_type() + " RESULT = null;");

      /*
       * Add code to propagate RESULT assignments that occur in action code embedded in a production
       * (ie, non-rightmost action code). 24-Mar-1998 CSA
       */
      for (int i = 0; i < prod.rhs_length(); i++) {
        ...
        out.println("              " + "if ( " + "((java_cup.runtime.Symbol) " + emit.pre("stack")
            + ".elementAt(" + emit.pre("top") + "-" + index + ")).value != null )");
      ...
      /*
       * here we have the left and right values being propagated. must make this a command line
       * option. frankf 6/18/96
       */

      /*
       * Create the code that assigns the left and right values of the new Symbol that the
       * production is reducing to
       */
      if (emit.lr_values()) {
        int loffset;
        ...
        out.println("              " + pre("result") + " = new java_cup.runtime.Symbol("
            + prod.lhs().the_symbol().index() + "/*" + prod.lhs().the_symbol().name() + "*/" + ", "
        ...
        if (act.kind() != parse_action.ERROR) {
          ...
          /* shouldn't be anything else */ else {
            throw new internal_error(
                "Unrecognized action code " + act.kind() + " found in parse table");
          }
        }
      }
  ...
  public static void parser(PrintWriter out, parse_action_table action_table,
      parse_reduce_table reduce_table, int start_st, production start_prod, boolean compact_reduces,
